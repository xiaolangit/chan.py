...
def create_complete_multi_timeframe_chart():
    """
    创建完整模仿图1效果的4个时间框架缠论分析图表
    包含完整的买卖点箭头和标签显示
    """
    ...
    for i, (name, kl_type) in enumerate(timeframes):
        ...
        try:
            ...
            # 绘制缠论元素
            y_min, y_max = draw_chan_elements_complete(meta, ax, plot_config, x_limits, y_min, y_max)
            
            # 添加一些边距
            y_margin = (y_max - y_min) * 0.1  # 增加边距以容纳买卖点箭头
    ...
    # 保存图表
    filename = f'./complete_multi_timeframe_{code.replace(".", "_")}.png'
    plt.savefig(filename, dpi=300, bbox_inches='tight', facecolor='white')
    print(f"\n✓ Complete multi-timeframe chart saved as: {filename}")
...
def draw_chan_elements_complete(meta, ax, plot_config, x_limits, y_min, y_max):
    """
    绘制完整的缠论分析元素，模仿图1的效果
    """
    x_begin = x_limits[0]
    ...
    if plot_config.get("plot_kline", False):
        draw_klines_complete(meta, ax, x_begin)
    
    # 绘制合并K线
    if plot_config.get("plot_kline_combine", False):
        draw_combined_klines_complete(meta, ax, x_begin)
    
    # 绘制笔
    if plot_config.get("plot_bi", False):
        draw_bi_lines_complete(meta, ax, x_begin)
    
    # 绘制段
    if plot_config.get("plot_seg", False):
        draw_seg_lines_complete(meta, ax, x_begin)
    
    # 绘制中枢
    if plot_config.get("plot_zs", False):
        draw_zs_boxes_complete(meta, ax, x_begin)
    
    # 绘制买卖点（完整版本，模仿图1）
    if plot_config.get("plot_bsp", False):
        y_min, y_max = draw_buy_sell_points_complete(meta, ax, x_begin, y_min, y_max)
    
    return y_min, y_max

def draw_klines_complete(meta, ax, x_begin):
    """绘制K线 - 完整版本"""
    try:
        for kl in meta.klu_iter():
            ...
            ax.bar(i, height, bottom=bottom, width=0.6, color=color, alpha=0.8, edgecolor='black', linewidth=0.5)
            
            # 绘制影线
            ax.plot([i, i], [kl.low, kl.high], color='black', linewidth=1)
    except Exception as e:
        print(f"绘制K线时出错: {e}")

def draw_combined_klines_complete(meta, ax, x_begin):
    """绘制合并K线 - 完整版本"""
    try:
...
def draw_bi_lines_complete(meta, ax, x_begin):
    """绘制笔 - 完整版本"""
    try:
        if hasattr(meta, 'bi_list') and meta.bi_list:
            for bi in meta.bi_list:
                ...
                ax.scatter([bi.begin_x, bi.end_x], [bi.begin_y, bi.end_y], 
                          color='blue', s=40, zorder=5, edgecolors='white', linewidth=1)
    except Exception as e:
        print(f"绘制笔时出错: {e}")

def draw_seg_lines_complete(meta, ax, x_begin):
    """绘制段 - 完整版本"""
    try:
        if hasattr(meta, 'seg_list') and meta.seg_list:
            for seg in meta.seg_list:
                ...
                ax.plot([seg.begin_x, seg.end_x], [seg.begin_y, seg.end_y], 
                       color='purple', linewidth=3.5, alpha=0.8, zorder=4)
    except Exception as e:
        print(f"绘制段时出错: {e}")

def draw_zs_boxes_complete(meta, ax, x_begin):
    """绘制中枢 - 完整版本"""
    try:
        if hasattr(meta, 'zs_lst') and meta.zs_lst:
            ...
            for zs in meta.zs_lst:
                ...
                rect = Rectangle((zs.begin, zs.low), width, height, 
                               ...
                               facecolor='yellow', alpha=0.4, zorder=2)
                ax.add_patch(rect)
    except Exception as e:
        print(f"绘制中枢时出错: {e}")

def draw_buy_sell_points_complete(meta, ax, x_begin, y_min, y_max):
    """绘制买卖点 - 完整版本，模仿图1的箭头和标签效果"""
    try:
        if hasattr(meta, 'bs_point_lst') and meta.bs_point_lst:
            ...
            for bsp in meta.bs_point_lst:
                ...
                ax.arrow(bsp.x, text_y, 0, (arrow_len - arrow_head) * arrow_dir,
                        ...
                        color=color, alpha=0.8, linewidth=2, zorder=10)
                
                # 更新Y轴范围以包含文本
                if text_y < y_min:
                    y_min = text_y - arrow_len * 0.1
                if text_y > y_max:
                    y_max = text_y + arrow_len * 0.1
            
            # 绘制段买卖点
            if hasattr(meta, 'seg_bsp_lst') and meta.seg_bsp_lst:
                arrow_l_seg = 0.2    # 段买卖点箭头稍长一些
                fontsize_seg = 14    # 段买卖点字体稍大一些
                arrow_w_seg = 1.5    # 段买卖点箭头稍粗一些
                ...
                for seg_bsp in meta.seg_bsp_lst:
                    ...
                    ax.arrow(seg_bsp.x, text_y, 0, (arrow_len - arrow_head) * arrow_dir,
                            ...
                            color=color, alpha=0.9, linewidth=2.5, zorder=11)
...
if __name__ == "__main__":
    print("=== 创建完整多时间框架缠论分析图表 ===")
    print("正在生成4个时间框架在一张图上的效果...")
    print("时间框架: 1分钟、5分钟、15分钟、1天")
    print("布局: 2x2 子图布局")
    print("特性: 完整买卖点箭头和标签显示（模仿图1效果）")
    print()
    
    try:
        fig = create_complete_multi_timeframe_chart()
        print("\n✓ 完整统一图表创建成功！")
        print("请查看生成的图片文件。")
